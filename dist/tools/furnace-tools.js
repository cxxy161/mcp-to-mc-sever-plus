...
export function registerFurnaceTools(factory, getBot) {
    factory.registerTool("smelt-item", "Smelt items using a furnace-like block", {
    ...
    }, async ({ x, y, z, inputItem, inputCount = 1, fuelItem, fuelCount = 1, takeOutput = true, timeoutMs = 60000 }) => {
        ...
        try {
            ...
            if (!takeOutput) {
                return factory.createResponse(`Started smelting ${resolvedInputCount} ${input.name} with ${resolvedFuelCount} ${fuel.name}`);
            }
...
async function waitForOutput(furnace, timeoutMs) {
